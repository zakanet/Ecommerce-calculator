...
export default async function handler(req, res) {
  ...
  const allowed = [
    'https://hasibpro-github-io-wy99.vercel.app',
    'http://localhost:3000'
  ];
  ...
  // ✅ 4. Auth check — log but don't hard-block (token may not be ready)
  const authHeader = req.headers['authorization'] || '';
  const token = authHeader.replace('Bearer ', '').trim();
  // Note: We log missing tokens but allow request to proceed
  // Full auth enforcement is handled by Supabase RLS
  if (!token) {
    console.warn('[HasibPro] No auth token — request allowed via RLS');
  }
